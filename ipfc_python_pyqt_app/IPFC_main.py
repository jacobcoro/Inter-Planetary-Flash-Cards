...
class Login(QDialog):
    ...
    # def get_nonce(self, email):
    #     url = api_url + "getnonce"
    ...
    def verify_login(self):
        ...
        # trial_key = argon2.hash(password=entered_password, salt=stored_salt, t=16, m=512, p=2, buflen=64).hex()
        # url = api_url + "verifyloginold"
        # form_data = {"email": self.entered_email, "key": str(trial_key)}

        form_data = {"email": self.entered_email, "password": entered_password}
        req = requests.get(url, data=form_data)
        api_response = json.loads(req.text)
    ...
    def media_IPFS_uploader(self, deck_file_path, uploaded_file_names, replace_links='no'):
        # In the future, make this optional to do every time or not
        """upload all local media files to the IPFS. The returning CIDS will be stored in in media folder
...
class Signup(QDialog):
    ...
    def verify_signup(self):
        def create_key_and_salt(plain_text_password):
            salt = bcrypt.gensalt(10)
            new_key = bcrypt.hashpw(plain_text_password.encode('utf-8'), salt)
            return [new_key, salt]
...
#        initial_pairs = [pair for pair in self.deck_dict.items()]
    ...
    def game_over(self):
        ...
        self.ui.QlabelCorrect.setText("Your score was " + str(round(100 * self.score / self.top_score, 2)) + "%. "
                                      "You got " + str(self.score) + "/" + str(self.top_score) + " cards correct.")
